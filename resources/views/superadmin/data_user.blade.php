...
@section('dashboard-content')
    <div class="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-sm border border-white/50 mb-8" data-aos="fade-up">
        <div class="flex flex-col gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Data User & Penyewa 👤</h1>
                <p class="text-gray-500 text-sm">Kelola dan pantau seluruh user serta penyewa (Anak Kos) yang terdaftar.
                </p>
            </div>
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-2 p-1 bg-gray-100 rounded-xl w-fit">
                    <a href="{{ route('superadmin.data_user', ['type' => 'all', 'search' => $search]) }}"
                        class="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all {{ $currentType === 'all' ? 'bg-white text-[#36B2B2] shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        Semua
                    </a>
                    <a href="{{ route('superadmin.data_user', ['type' => 'penyewa', 'search' => $search]) }}"
                        class="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all {{ $currentType === 'penyewa' ? 'bg-white text-[#36B2B2] shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        Penyewa
                    </a>
                    <a href="{{ route('superadmin.data_user', ['type' => 'user', 'search' => $search]) }}"
                        class="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all {{ $currentType === 'user' ? 'bg-white text-[#36B2B2] shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        User Biasa
                    </a>
                </div>
                <div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    @if($user->isPenyewa())
                                        <div class="flex flex-col gap-1">
                                            <span
                                                class="w-fit px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase border border-emerald-100">
                                                Penyewa Aktif
                                            </span>
                                            <div class="text-[9px] text-gray-400 font-bold uppercase tracking-tight">Kamar:
                                                {{ $user->kamar->nomor_kamar ?? '-' }}</div>
                                        </div>
                                    @else
                                        <span
                                            class="px-3 py-1 bg-gray-100 text-gray-500 rounded-full text-[10px] font-black uppercase border border-gray-200">
                                            User Biasa
                                        </span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-400 italic">Belum ada user terdaftar.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
