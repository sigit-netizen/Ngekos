...
@section('dashboard-content')
            ...
            <form action="{{ route('superadmin.data_member') }}" method="GET">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <select name="paket"
                        ...
                        <option value="2" {{ ($paket ?? '') == '2' ? 'selected' : '' }}>PRO</option>
                        <option value="3" {{ ($paket ?? '') == '3' ? 'selected' : '' }}>PREMIUM</option>
                        <option value="4" {{ ($paket ?? '') == '4' ? 'selected' : '' }}>PREMIUM PER KAMAR</option>
                ...
                @if($search || $paket)
                    <div class="flex flex-wrap gap-2 mt-3">
                        @if($search)
                            <span
                                style="padding:4px 12px; background:#e0f2f1; color:#0f766e; font-size:12px; font-weight:700; border-radius:99px;">
                                🔎 "{{ $search }}"
                            </span>
                        @endif
                        @if($paket)
                            @php $paketNames = [2 => 'PRO', 3 => 'PREMIUM', 4 => 'PREMIUM PER KAMAR', 5 => 'PRO PER KAMAR']; @endphp
                            <span
                                style="padding:4px 12px; background:#e0f7fa; color:#0e7490; font-size:12px; font-weight:700; border-radius:99px;">
                                📦 Paket: {{ $paketNames[(int) $paket] ?? $paket }}
                            </span>
                        @endif
                    </div>
                @endif
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-400 italic">Belum ada member terdaftar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="editMemberModal"
        ...
        <div
            ...
            <div
                ...
                <div>
                    <h3 style="font-size:18px; font-weight:800; color:#0f172a; margin:0;">Edit Member</h3>
                    <p style="font-size:12px; color:#94a3b8; margin:4px 0 0 0;">Perbarui data pemilik kos</p>
                </div>
            ...
            <form id="editForm" method="POST" style="padding:24px 28px;">
                ...
                <div style="margin-bottom:24px;">
                    <label
                        ...
                        Paket</label>
                    <select name="id_plans" id="edit_plans" required
                        style="width:100%; padding:10px 14px; border:2px solid #e2e8f0; border-radius:12px; font-size:14px; color:#1e293b; background:#f8fafc; outline:none; cursor:pointer; box-sizing:border-box;">
                        <option value="2">🚀 PRO</option>
                        <option value="3">⭐ PREMIUM</option>
                        <option value="4">🏆 PREMIUM PER KAMAR</option>
                        <option value="5">💎 PRO PER KAMAR</option>
                    </select>
                </div>
    ...
    <script>
        ...
        function editMember(member) {
            document.getElementById('editForm').action = `/superadmin/data-member/${member.id}`;
            document.getElementById('edit_name').value = member.name;
            document.getElementById('edit_email').value = member.email;
            document.getElementById('edit_nik').value = member.nik;
            document.getElementById('edit_wa').value = member.nomor_wa;
            document.getElementById('edit_plans').value = member.id_plans;
